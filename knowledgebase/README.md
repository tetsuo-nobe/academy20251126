...
1. **バケット名**は下記のようにしてしてください。
    - `tnobect-academy-kb-ご自分の番号`
    - 例: tnobect-academy-kb-99
...
1. **S3 URI** に下記のように入力します。
    - `s3://tnobect-academy-kb-自分の番号`
    - 例: s3://tnobect-academy-kb-99
...
1. AWS マネジメントコンソールのページ上部の **検索**で `s3` と入力して **S3** のメニューを選択します。
1. バケットの名前で、**tnobect-academy-kb-ご自分の番号** の横のラジオボタンを選択します。
1. **空にする** を選択します。
